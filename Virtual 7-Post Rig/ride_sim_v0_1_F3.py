import sys
import csv
from PyQt5.QtWidgets import QApplication, QWidget, QGroupBox, QPushButton, QFormLayout, QLineEdit, QToolButton, QLabel, QFileDialog, QSpinBox, QTreeWidget, QTreeWidgetItem, QMessageBox, QComboBox
...
def run_simulation(args):
    ...
    parameters, track = args

    frh0, rrh0, mCar, rWeightBalF, Icar, mHubF, mHubR, kinstf, kinstr, zCoG, wheelbase, tF, tR, kspringf, Speed_F, ForceD_F, Disp_F, ForceB_F, freegapf, karbf, kspringr, Speed_R, ForceD_R, Disp_R, ForceB_R, freegapr, karbr, fa0, fa01, fa02, fa03, fa10, fa20, fa30, fa11, fa12, fa21, fa22, faflap10, faflap20, faflap01, faflap02, ra0, ra01, ra02, ra03, ra10, ra20, ra30, ra11, ra12, ra21, ra22, raflap10, raflap20, raflap01, raflap02, a0, a01, a02, a03, a10, a20, a30, a11, a12, a21, a22, aflap10, aflap20, aflap01, aflap02, aero_frontflap, aero_rearflap, ktf, ktr = parameters

    times, distances, rPedal, pBrake, ztrackFL, ztrackFR, ztrackRL, ztrackRR, Ax, Ay, vCar = track
    ...
    def vehicle_model(t, z, frh0, rrh0):
        ...
        h_2dot = ((1/(1/(kFR
                 ...
                 - 0.5*(cl_front + cl_rear)*rho*((vx/3.6)**2))/Ms
        ...
        phi_2dot = (lr*((1/(1/(kRR
                 ...
                 - (h - zCoG)*0.5*cd*rho*((vx/3.6)**2))/Iyy
    ...
    index_speed = np.linspace(0, len(times) - 1, len(vCar))
    index_ax = np.linspace(0, len(times) - 1, len(Ax))
    index_ay = np.linspace(0, len(times) - 1, len(Ay))
    index_pedal = np.linspace(0, len(times) - 1, len(rPedal))
    index_pbrake = np.linspace(0, len(times) - 1, len(pBrake))

    interp_func_speed = interp1d(index_speed, vCar, kind = 'linear', fill_value = 'extrapolate')
    vCar = interp_func_speed(np.arange(len(times)))

    interp_func_ax = interp1d(index_ax, Ax, kind = 'linear', fill_value = 'extrapolate')
    Ax = interp_func_ax(np.arange(len(times)))

    interp_func_ay = interp1d(index_ay, Ay, kind = 'linear', fill_value = 'extrapolate')
    Ay = interp_func_ay(np.arange(len(times)))

    interp_func_pedal = interp1d(index_pedal, rPedal, kind = 'linear', fill_value = 'extrapolate')
    rPedal = interp_func_pedal(np.arange(len(times)))

    interp_func_pbrake = interp1d(index_pbrake, pBrake, kind = 'linear', fill_value = 'extrapolate')
    pBrake = interp_func_pbrake(np.arange(len(times)))
...
def run_sweep(self):
    ...
    sweep_parameter_value = re.findall(r"-?\d+\.\d+|-?\d+", self.sweep_values.text())
    
    sweep_parameters = [parameters.copy() for _ in range(len(sweep_parameter_value))]

    def list_to_float(lista):
        if isinstance(lista, list):
            return [list_to_float(item) for item in lista]
        else:
            return float(lista) if isinstance(lista, str) else lista
        
    sweep_parameters = list_to_float(sweep_parameters)
    sweep_parameter_value = list_to_float(sweep_parameter_value)

    if self.combo_box_sim.currentText() == 'Absolute':
        for i in range(len(sweep_parameter_value)):
            if self.combo_box_parameter.currentText() == 'Chassis_hRideFSetup':
                sweep_parameters[i][0] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_hRideRSetup':
                sweep_parameters[i][1] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_carRunningMass_mCar':
                sweep_parameters[i][2] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_carRunningMass_rWeightBalF':
                sweep_parameters[i][3] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Icar':
                sweep_parameters[i][4] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_mHubF':
                sweep_parameters[i][5] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_mHubR':
                sweep_parameters[i][6] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_kVerticalSuspensionComplianceF':
                sweep_parameters[i][7] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_kVerticalSuspensionComplianceR':
                sweep_parameters[i][8] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_zCoG':
                sweep_parameters[i][9] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Wheelbase':
                sweep_parameters[i][10] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Front Track':
                sweep_parameters[i][11] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Rear Track':
                sweep_parameters[i][12] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_Spring_kSpring':
                sweep_parameters[i][13] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_Damper_Speed':
                sweep_parameters[i][14] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_Damper_Force':
                sweep_parameters[i][15] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_BumpStop_Displacement':
                sweep_parameters[i][16] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_BumpStop_Force':
                sweep_parameters[i][17] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_BumpStop_xFreeGap':
                sweep_parameters[i][18] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_antiRollBar_kAntiRollBar':
                sweep_parameters[i][19] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_Spring_kSpring':
                sweep_parameters[i][20] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_Damper_Speed':
                sweep_parameters[i][21] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_Damper_Force':
                sweep_parameters[i][22] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_BumpStop_Displacement':
                sweep_parameters[i][23] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_BumpStop_Force':
                sweep_parameters[i][24] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_BumpStop_xFreeGap':
                sweep_parameters[i][25] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_antiRollBar_kAntiRollBar':
                sweep_parameters[i][26] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_Const':
                sweep_parameters[i][27] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideR':
                sweep_parameters[i][28] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideR*hRideR':
                sweep_parameters[i][29] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideR*hRideR*hRideR':
                sweep_parameters[i][30] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF':
                sweep_parameters[i][31] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF':
                sweep_parameters[i][32] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF*hRideF':
                sweep_parameters[i][33] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideR':
                sweep_parameters[i][34] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideR*hRideR':
                sweep_parameters[i][35] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF*hRideR':
                sweep_parameters[i][36] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF*hRideR*hRideR':
                sweep_parameters[i][37] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapF':
                sweep_parameters[i][38] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapF*aFlapF':
                sweep_parameters[i][39] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapR':
                sweep_parameters[i][40] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapR*aFlapR':
                sweep_parameters[i][41] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_Const':
                sweep_parameters[i][42] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideR':
                sweep_parameters[i][43] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideR*hRideR':
                sweep_parameters[i][44] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideR*hRideR*hRideR':
                sweep_parameters[i][45] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF':
                sweep_parameters[i][46] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF':
                sweep_parameters[i][47] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF*hRideF':
                sweep_parameters[i][48] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideR':
                sweep_parameters[i][49] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideR*hRideR':
                sweep_parameters[i][50] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF*hRideR':
                sweep_parameters[i][51] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF*hRideR*hRideR':
                sweep_parameters[i][52] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapF':
                sweep_parameters[i][53] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapF*aFlapF':
                sweep_parameters[i][54] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapR':
                sweep_parameters[i][55] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapR*aFlapR':
                sweep_parameters[i][56] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_Const':
                sweep_parameters[i][57] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideR':
                sweep_parameters[i][58] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideR*hRideR':
                sweep_parameters[i][59] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideR*hRideR*hRideR':
                sweep_parameters[i][60] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF':
                sweep_parameters[i][61] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF':
                sweep_parameters[i][62] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF*hRideF':
                sweep_parameters[i][63] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideR':
                sweep_parameters[i][64] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideR*hRideR':
                sweep_parameters[i][65] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF*hRideR':
                sweep_parameters[i][66] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF*hRideR*hRideR':
                sweep_parameters[i][67] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapF':
                sweep_parameters[i][68] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapF*aFlapF':
                sweep_parameters[i][69] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapR':
                sweep_parameters[i][70] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapR*aFlapR':
                sweep_parameters[i][71] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_Flaps_aFlapF':
                sweep_parameters[i][72] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_Flaps_aFlapR':
                sweep_parameters[i][73] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Tyres_Front_kVerticalTyre':
                sweep_parameters[i][74] = sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Tyres_Rear_kVerticalTyre':
                sweep_parameters[i][75] = sweep_parameter_value[i]

    if self.combo_box_sim.currentText() == 'Additive':
        for i in range(len(sweep_parameter_value)):
            if self.combo_box_parameter.currentText() == 'Chassis_hRideFSetup':
                sweep_parameters[i][0] = sweep_parameters[i][0] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_hRideRSetup':
                sweep_parameters[i][1] = sweep_parameters[i][1] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_carRunningMass_mCar':
                sweep_parameters[i][2] = sweep_parameters[i][2] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_carRunningMass_rWeightBalF':
                sweep_parameters[i][3] = sweep_parameters[i][3] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Icar':
                sweep_parameters[i][4] = sweep_parameters[i][4] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_mHubF':
                sweep_parameters[i][5] = sweep_parameters[i][5] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_mHubR':
                sweep_parameters[i][6] = sweep_parameters[i][6] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_kVerticalSuspensionComplianceF':
                sweep_parameters[i][7] = sweep_parameters[i][7] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_kVerticalSuspensionComplianceR':
                sweep_parameters[i][8] = sweep_parameters[i][8] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_zCoG':
                sweep_parameters[i][9] = sweep_parameters[i][9] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Wheelbase':
                sweep_parameters[i][10] = sweep_parameters[i][10] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Front Track':
                sweep_parameters[i][11] = sweep_parameters[i][11] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Chassis_Rear Track':
                sweep_parameters[i][12] = sweep_parameters[i][12] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_Spring_kSpring':
                sweep_parameters[i][13] = sweep_parameters[i][13] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_Damper_Speed':
                sweep_parameters[i][14] = sweep_parameters[i][14] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_Damper_Force':
                sweep_parameters[i][15] = sweep_parameters[i][15] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_BumpStop_Displacement':
                sweep_parameters[i][16] = sweep_parameters[i][16] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_BumpStop_Force':
                sweep_parameters[i][17] = sweep_parameters[i][17] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_BumpStop_xFreeGap':
                sweep_parameters[i][18] = sweep_parameters[i][18] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Front_Internal_antiRollBar_kAntiRollBar':
                sweep_parameters[i][19] = sweep_parameters[i][19] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_Spring_kSpring':
                sweep_parameters[i][20] = sweep_parameters[i][20] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_Damper_Speed':
                sweep_parameters[i][21] = sweep_parameters[i][21] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_Damper_Force':
                sweep_parameters[i][22] = sweep_parameters[i][22] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_BumpStop_Displacement':
                sweep_parameters[i][23] = sweep_parameters[i][23] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_BumpStop_Force':
                sweep_parameters[i][24] = sweep_parameters[i][24] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_BumpStop_xFreeGap':
                sweep_parameters[i][25] = sweep_parameters[i][25] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Suspension_Rear_Internal_antiRollBar_kAntiRollBar':
                sweep_parameters[i][26] = sweep_parameters[i][26] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_Const':
                sweep_parameters[i][27] = sweep_parameters[i][27] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideR':
                sweep_parameters[i][28] = sweep_parameters[i][28] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideR*hRideR':
                sweep_parameters[i][29] = sweep_parameters[i][29] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideR*hRideR*hRideR':
                sweep_parameters[i][30] = sweep_parameters[i][30] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF':
                sweep_parameters[i][31] = sweep_parameters[i][31] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF':
                sweep_parameters[i][32] = sweep_parameters[i][32] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF*hRideF':
                sweep_parameters[i][33] = sweep_parameters[i][33] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideR':
                sweep_parameters[i][34] = sweep_parameters[i][34] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideR*hRideR':
                sweep_parameters[i][35] = sweep_parameters[i][35] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF*hRideR':
                sweep_parameters[i][36] = sweep_parameters[i][36] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_hRideF*hRideF*hRideR*hRideR':
                sweep_parameters[i][37] = sweep_parameters[i][37] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapF':
                sweep_parameters[i][38] = sweep_parameters[i][38] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapF*aFlapF':
                sweep_parameters[i][39] = sweep_parameters[i][39] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapR':
                sweep_parameters[i][40] = sweep_parameters[i][40] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyF_aFlapR*aFlapR':
                sweep_parameters[i][41] = sweep_parameters[i][41] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_Const':
                sweep_parameters[i][42] = sweep_parameters[i][42] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideR':
                sweep_parameters[i][43] = sweep_parameters[i][43] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideR*hRideR':
                sweep_parameters[i][44] = sweep_parameters[i][44] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideR*hRideR*hRideR':
                sweep_parameters[i][45] = sweep_parameters[i][45] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF':
                sweep_parameters[i][46] = sweep_parameters[i][46] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF':
                sweep_parameters[i][47] = sweep_parameters[i][47] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF*hRideF':
                sweep_parameters[i][48] = sweep_parameters[i][48] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideR':
                sweep_parameters[i][49] = sweep_parameters[i][49] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideR*hRideR':
                sweep_parameters[i][50] = sweep_parameters[i][50] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF*hRideR':
                sweep_parameters[i][51] = sweep_parameters[i][51] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_hRideF*hRideF*hRideR*hRideR':
                sweep_parameters[i][52] = sweep_parameters[i][52] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapF':
                sweep_parameters[i][53] = sweep_parameters[i][53] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapF*aFlapF':
                sweep_parameters[i][54] = sweep_parameters[i][54] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapR':
                sweep_parameters[i][55] = sweep_parameters[i][55] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CLiftBodyR_aFlapR*aFlapR':
                sweep_parameters[i][56] = sweep_parameters[i][56] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_Const':
                sweep_parameters[i][57] = sweep_parameters[i][57] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideR':
                sweep_parameters[i][58] = sweep_parameters[i][58] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideR*hRideR':
                sweep_parameters[i][59] = sweep_parameters[i][59] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideR*hRideR*hRideR':
                sweep_parameters[i][60] = sweep_parameters[i][60] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF':
                sweep_parameters[i][61] = sweep_parameters[i][61] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF':
                sweep_parameters[i][62] = sweep_parameters[i][62] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF*hRideF':
                sweep_parameters[i][63] = sweep_parameters[i][63] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideR':
                sweep_parameters[i][64] = sweep_parameters[i][64] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideR*hRideR':
                sweep_parameters[i][65] = sweep_parameters[i][65] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF*hRideR':
                sweep_parameters[i][66] = sweep_parameters[i][66] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_hRideF*hRideF*hRideR*hRideR':
                sweep_parameters[i][67] = sweep_parameters[i][67] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapF':
                sweep_parameters[i][68] = sweep_parameters[i][68] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapF*aFlapF':
                sweep_parameters[i][69] = sweep_parameters[i][69] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapR':
                sweep_parameters[i][70] = sweep_parameters[i][70] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_CDragBody_aFlapR*aFlapR':
                sweep_parameters[i][71] = sweep_parameters[i][71] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_Flaps_aFlapF':
                sweep_parameters[i][72] = sweep_parameters[i][72] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Aero_Flaps_aFlapR':
                sweep_parameters[i][73] = sweep_parameters[i][73] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Tyres_Front_kVerticalTyre':
                sweep_parameters[i][74] = sweep_parameters[i][74] + sweep_parameter_value[i]

            elif self.combo_box_parameter.currentText() == 'Tyres_Rear_kVerticalTyre':
                sweep_parameters[i][75] = sweep_parameters[i][75] + sweep_parameter_value[i]


    if self.combo_box_sim.currentText() == 'Multiplicative':
        for i in range(len(sweep_parameter_value)):
            if self.combo_box_parameter.currentText() == 'Chassis_hRideFSetup':
                sweep_parameters[i][0] = sweep_parameters[i][0]*sweep_parameter_value[i]
    ...
    args = zip(sweep_parameters, itertools.repeat(track))

    with ProcessPoolExecutor(max_workers=len(sweep_parameter_value)) as executor:
        ...
        with open(csv_filename, mode='w', newline='') as file:
            ...
            interp_ztrackRL = interp1d(t_delay, track[4], bounds_error=False, fill_value='extrapolate')
            ztrackRL = interp_ztrackRL(track[0])

            interp_ztrackRR = interp1d(t_delay, track[5], bounds_error=False, fill_value='extrapolate')
            ...
            for row_idx in range(max_len):
                ...
                d_values = [d[row_idx] if row_idx < len(d) else 'Nan' for d in track[1]]
                row.extend([d_values])
                ...
                for i in range(len(results)):
                    for j in range(15):
                        if j == 0:
                            ...
                            value = track[3][row_idx]
                        elif j == 2:
                            value = track[4][row_idx]
                        elif j == 3:
                            value = track[5][row_idx]
                        elif j == 4:
                            ...
                            value = parameters[74]*(track[4][row_idx] - y_list[i][8][row_idx])
                        elif j == 12:
                            value = parameters[74]*(track[5][row_idx] - y_list[i][6][row_idx])                        
                        elif j == 13:
                            value = parameters[75]*(ztrackRL[row_idx] - y_list[i][10][row_idx])
                        elif j == 14:
                            value = parameters[75]*(ztrackRR[row_idx] - y_list[i][12][row_idx])
...
class RideSimApp(QWidget):
    def __init__(self):
        ...
        self.label.move(1100, 200)
        self.label.show()

        self.label = QLabel('Select parameter to be sweeped:', self)
        self.label.setFont(QFont('Arial', 12))
        self.label.move(1100, 250)
        self.label.show()

        self.combo_box_parameter = QComboBox(self)
        self.combo_box_parameter.move(1350, 250)
        self.combo_box_parameter.setFixedWidth(250)
        self.combo_box_parameter.setEditable(True)  
        self.combo_box_parameter.completer().setFilterMode(Qt.MatchContains)
        self.combo_box_parameter.completer().setCaseSensitivity(Qt.CaseInsensitive)
        self.combo_box_parameter.show()

        self.label = QLabel('Select simulation sweep type:', self)
        self.label.setFont(QFont('Arial', 12))
        self.label.move(1100, 300)
        self.label.show()

        self.combo_box_sim = QComboBox(self)
        self.combo_box_sim.move(1350, 300)
        self.combo_box_sim.addItems(['Absolute', 'Additive', 'Multiplicative'])
        self.combo_box_sim.setFixedWidth(250)
        self.combo_box_sim.show()

        self.label = QLabel('Input Type:', self)
        self.label.setFont(QFont('Arial', 12))
        self.label.move(1100, 350)
        self.label.show()

        self.combo_box_type = QComboBox(self)
        self.combo_box_type.move(1350, 350)
        self.combo_box_type.addItems(['Numerical Values', 'Numerical Array Values', 'Saved Configs'])
        self.combo_box_type.setFixedWidth(250)
        self.combo_box_type.show()

        self.label = QLabel('', self)
        self.label.setFont(QFont('Arial', 12))
        self.label.move(1100, 400)
        self.label.show()

        self.combo_box_type.currentIndexChanged.connect(self.update_label)

        self.update_label()

        self.sweep_values = QLineEdit('', self)
        self.sweep_values.move(1350, 400)
        self.sweep_values.setFixedWidth(250)
        self.sweep_values.setStyleSheet('border-radius: 5px; padding: 3px; background-color: #2F2F2F; color: #ddd;')
        self.sweep_values.show()

        self.sweep_name = QLineEdit('', self)
        self.sweep_name.move(1350, 200)
        self.sweep_name.setFixedWidth(250)
        ...
        self.damper_front_item = QTreeWidgetItem(self.internal_front_suspension_item, ['Damper'])
        ...
        self.damper_rear_item = QTreeWidgetItem(self.internal_rear_suspension_item, ['Damper'])
        ...
        self.add_editable_child(self.damper_front_item, 'Speed')
        self.add_editable_child(self.damper_front_item, 'Force')
        ...
        self.add_editable_child(self.damper_rear_item, 'Speed')
        self.add_editable_child(self.damper_rear_item, 'Force')
    ...
    def update_label(self):
        ...
        selected_text = self.combo_box_type.currentText()

        if selected_text == 'Numerical Values':
            text = 'Numerical values:'
        elif selected_text == 'Numerical Array Values':
            text = 'Numerical array values:'
        elif selected_text == 'Saved Configs':
            text = 'Saved Configs:'
        else:
            text = ''
    ...
    def add_editable_child(self, parent_item, name):
        ...
        line_edit.textChanged.connect(lambda text, key=unique_name: self.update_car_data(key, text))

        parent_item.treeWidget().setItemWidget(child_item, 1, line_edit)
        
        child_item.setText(0, name)

        if hasattr(self, 'combo_box_parameter'):
            self.combo_box_parameter.addItem(unique_name)
    ...
    def load_json(self):
        ...
        self.set_line_edit_value('Suspension_Front_Internal_Damper', 'Force', self.car_data.get('config').get('suspension', {}).get('front', {}).get('internal', {}).get('damper', {}).get('FDamperLU', ''))
        self.set_line_edit_value('Suspension_Front_Internal_Damper', 'Speed', self.car_data.get('config').get('suspension', {}).get('front', {}).get('internal', {}).get('damper', {}).get('vDamperBasis', ''))
        ...
        self.set_line_edit_value('Suspension_Rear_Internal_Damper', 'Force', self.car_data.get('config').get('suspension', {}).get('rear', {}).get('internal', {}).get('damper', {}).get('FDamperLU', ''))
        self.set_line_edit_value('Suspension_Rear_Internal_Damper', 'Speed', self.car_data.get('config').get('suspension', {}).get('rear', {}).get('internal', {}).get('damper', {}).get('vDamperBasis', ''))
        ...
        if archive:
            try:
                with open(archive, mode='r') as file:
                    ...
                    self.distance = [float(row[1]) for row in values]
                    self.pBrake = [float(row[2]) for row in values]
                    self.rPedal = [float(row[3]) for row in values]
                    self.ztrackFL = [float(row[4]) for row in values]
                    self.ztrackFR = [float(row[5]) for row in values]
                    self.ztrackRL = [float(row[6]) for row in values]
                    self.ztrackRR = [float(row[7]) for row in values]
                    self.Ax = [float(row[8]) for row in values]
                    self.Ay = [float(row[9]) for row in values]
                    self.vCar = [float(row[10]) for row in values]
