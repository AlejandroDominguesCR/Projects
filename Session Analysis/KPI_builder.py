...
def compute_top_speeds(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "top_speed" not in df.columns:
        # Sesión sin dato de punta por vuelta (ej. timings de Jerez)
        # Devolvemos DataFrame vacío pero con la estructura esperada
        return pd.DataFrame(columns=["number", "team", "max_top_speed"])
...
def track_limit_rate(track_df: pd.DataFrame, laps_df: pd.DataFrame) -> pd.DataFrame:
    """Calcula la tasa de infracciones de track limi por vuelta.

    Parameters
    ----------
    track_df : pd.DataFrame
        DataFrame con incidencias de track limits. Típicamente el fichero
        ``99_TrackLimits_*.csv`` ya normalizado por `load_session_data`, con
        columnas como ``nr`` (dorsal), ``driver`` (nombre),
        ``total_infractions`` o ``incident``.
    laps_df : pd.DataFrame
        DataFrame con las vueltas completadas, típicamente el
        ``23_Analysis_*.csv`` ya normalizado, con columnas como
        ``number`` (``"Nombre #nr"``), ``driver_name``, ``lap_number``, ...

    Returns
    -------
    pd.DataFrame
        DataFrame con columnas:
        - ``number``   → etiqueta tipo ``"Nombre #nr"`` para usar en plots
        - ``incidents``→ nº total de track limits
        - ``laps``     → nº de vueltas completadas
        - ``rate``     → incidentes por vuelta
    """
    if track_df is None or track_df.empty:
        return pd.DataFrame(columns=["number", "incidents", "laps", "rate"])
    ...
    # 1) Normalizar columna de incidentes ---------------------------------
    if "incident" not in track_df.columns:
        if "total_infractions" in track_df.columns:
            ...
            # No hay info de incidentes → no se puede calcular nada sensato
            return pd.DataFrame(columns=["number", "incidents", "laps", "rate"])

    # 2) Construir clave de unión (join_key) -------------------------------
    # Caso típico EC3: tracklimits tiene 'nr' + 'driver'
    # y analysis tiene 'number' = "DriverName #nr"
    if {"nr", "driver"}.issubset(track_df.columns) and {"number", "driver_name"}.issubset(laps_df.columns):
        track_df["join_key"] = (
            track_df["driver"].astype(str).str.strip()
            + " #" + track_df["nr"].astype(str).str.strip()
        )
        laps_df["join_key"] = laps_df["number"].astype(str).str.strip()
    else:
        # Fallback genérico: buscar mejor columna de piloto común
        candidates = ["number", "driver_name", "driver", "driver_shortname", "driver_number"]
        track_driver = next((c for c in candidates if c in track_df.columns), None)
        lap_driver = next((c for c in candidates if c in laps_df.columns), None)
        if track_driver is None or lap_driver is None:
            return pd.DataFrame(columns=["number", "incidents", "laps", "rate"])

        track_df["join_key"] = track_df[track_driver].astype(str).str.strip()
        laps_df["join_key"] = laps_df[lap_driver].astype(str).str.strip()

    # 3) Columna de vuelta -------------------------------------------------
    lap_col = "lap_number" if "lap_number" in laps_df.columns else "lap"
    if lap_col not in laps_df.columns:
        return pd.DataFrame(columns=["number", "incidents", "laps", "rate"])

    # 4) Agregar: incidentes por piloto -----------------------------------
    incidents = (
        track_df.groupby("join_key")["incident"]
        .sum()
        .astype(float)
    )

    # 5) Agregar: nº de vueltas por piloto (contando vueltas únicas) ------
    laps = (
        laps_df.drop_duplicates(subset=["join_key", lap_col])
        .groupby("join_key")[lap_col]
        .count()
        .astype(float)
    )

    # 6) Unir resultados sobre el conjunto de claves ----------------------
    all_keys = sorted(set(incidents.index).union(laps.index))
    result = pd.DataFrame(
        {
            "number": all_keys,  # usamos join_key como etiqueta final
            "incidents": incidents.reindex(all_keys).fillna(0.0),
            "laps": laps.reindex(all_keys).fillna(0.0),
        }
    )

    # 7) Ratio y limpieza --------------------------------------------------
    result["rate"] = result["incidents"] / result["laps"].replace(0, np.nan)
    result["rate"] = result["rate"].fillna(0.0)

    # opcional: eliminar filas sin vueltas ni incidentes
    mask_nonzero = (result["incidents"] > 0) | (result["laps"] > 0)
    result = result[mask_nonzero].reset_index(drop=True)
...
def ideal_lap_gap(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_num.loc[df_num['lap_time'] <= 0, 'lap_time'] = np.nan
...
def best_sector_times(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Regla de selección de columnas:
      - Si existen columnas *_seconds (s1_seconds, sector1_seconds, ...) se usan SOLO esas.
      - Si no existen *_seconds, se usan columnas sin sufijo (s1, sector1, ...).

    En ambos casos se garantiza que solo haya UNA columna por sector (sector1, sector2, sector3)
    en el DataFrame resultado, para evitar nombres de columna duplicados.
    """
    if "number" not in df.columns:
        return pd.DataFrame()

    # 1) Detectamos posibles columnas de sector con sufijo *_seconds
    secs_pattern = re.compile(r"(?i)^(?:s|sector)([123])_seconds$")
    seconds_cols = [c for c in df.columns if secs_pattern.match(c)]

    # 2) Si no hay *_seconds, usamos columnas sin sufijo (s1, sector1, ...)
    if seconds_cols:
        candidates = seconds_cols
    else:
        nosuffix_pattern = re.compile(r"(?i)^(?:s|sector)([123])$")
        candidates = [c for c in df.columns if nosuffix_pattern.match(c)]

    if not candidates:
        return pd.DataFrame()

    # 3) Elegimos UNA sola columna por sector (1, 2, 3)
    #    Nos quedamos con la primera que aparezca en 'candidates'
    selected: dict[str, str] = {}
    for col in candidates:
        m = re.search(r"([123])", col)
        if not m:
            continue
        digit = m.group(1)  # "1", "2" o "3"
        if digit not in selected:
            selected[digit] = col  # guardamos solo la primera coincidencia

    if not selected:
        return pd.DataFrame()

    # 4) Orden consistente de sectores
    sector_order = ["1", "2", "3"]
    sector_cols = [selected[d] for d in sector_order if d in selected]

    # 5) Columnas base (piloto y, si existe, equipo)
    base_cols = ["number"] + (["team"] if "team" in df.columns else [])
    use_cols = base_cols + sector_cols

    df_sec = df[use_cols].copy()

    # 6) Pasamos a segundos lo que no sea numérico
    for col in sector_cols:
        if not pd.api.types.is_numeric_dtype(df_sec[col]):
            df_sec[col] = df_sec[col].apply(parse_time_to_seconds)

        # ⬅️ NUEVO: los tiempos 0.000 (o negativos) no son sectores válidos
        # (suelen ser out-lap, in-lap, etc.), los marcamos como NaN
        df_sec.loc[df_sec[col] <= 0, col] = np.nan

    # 7) Renombramos a sector1 / sector2 / sector3 SIN duplicar nombres
    rename_map = {
        selected[d]: f"sector{d}"
        for d in sector_order
        if d in selected
    }
    df_sec = df_sec.rename(columns=rename_map)

    # 8) Agrupamos por piloto (y equipo) y cogemos el mínimo por sector
    result = (
        df_sec
        .groupby(base_cols, as_index=False)[list(rename_map.values())]
        .min()
    )
...
def build_driver_tables(
    ...
    *,
    ...
    Ahora es robusta a:
      - falta de 'hour' (usa 'time' o timestamp sintético),
      - falta de 'lap_number' (la genera),
      - falta de 'top_speed' (la rellena con NaN).
    """
    ...
    # ── 1·5 asegurar lap_number ───────────────────────────────────────
    if "lap_number" not in df.columns:
        if "lap" in df.columns:
            df["lap_number"] = df["lap"]
        else:
            # enumerar por coche en el orden actual
            df["lap_number"] = df.groupby("number").cumcount() + 1

    # ── 2· normalizar tiempos ─────────────────────────────────────────
    if "lap_time" in df.columns and not pd.api.types.is_numeric_dtype(df["lap_time"]):
    ...
    if filter_fast and "lap_time" in df.columns:
    ...
    for drv, g in df.groupby("number"):
        ...
        missing = [c for c in base_cols if c not in g.columns]
        for c in missing:
            g[c] = np.nan
...
def build_fastest_lap_table(
    ...
    *,
    ...
    """
    Tabla de mejor vuelta por piloto, con GapStart, sectores y gaps de sector.
    Ahora tolera:
      - falta de 'hour' (usa 'time' o timestamp sintético),
      - falta de 'top_speed' (rellena con NaN).
    """
    ...
    if filter_fast:
        best = df.groupby("number")["lap_time"].transform("min")
        df = df[df["lap_time"] <= 1.2 * best].copy()
    ...
    # 5·5 asegurar top_speed
    if "top_speed" not in base.columns:
        base["top_speed"] = np.nan
    ...
    # 8 · filtrar teams (solo a nivel de presentación)
    if teams is not None and "team" in base.columns:
