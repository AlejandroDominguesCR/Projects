...
def parse_json_setup(json_data):
    ...
    mr_f = 1 #0.696 #0.696 1.437
    mr_r = 1 #0.75 #0.75 1.328
...
def simulate_combo(setup_path, track_path, kt_overrides=None):
    ...
    sol = run_vehicle_model_simple(t_vec, z_tracks, simple_params)
    simple_params['track_name'] = Path(track_path).stem
    post = postprocess_7dof(sol, simple_params, z_tracks, t_vec)
...
def load_track_channels(track_path):
    ...
    # Cambiamos a los nombres exactos de tu CSV
    columns_required = ['Time (s)', 'z_FL (m)', 'z_FR (m)', 'z_RL (m)', 'z_RR (m)']
    ...
    data = {
        # Nota: al cargar, renombramos 'Time (s)' a 't' y las z_* a z_tracks
        't': df['Time (s)'].values,
        'z_tracks': [
            df['z_FL (m)'].values,
            df['z_FR (m)'].values,
    ...
    # Si vienen en mm, conviertes a m (opcional)
    if max((abs(v).max() for v in data['z_tracks'])) > 2:
        data['z_tracks'] = [z / 1000.0 for z in data['z_tracks']]

    return data

class FourPostRigGUI(QWidget):
    
    def __init__(self):
        super().__init__()
        self.setWindowTitle("4-Post Rig Simulator")
    ...
    def add_tracks(self):
        ...
        for f in files:
            if f not in [self.track_list.item(i).text() for i in range(self.track_list.count())]:
                try:
                    df = pd.read_csv(f)
                    required = ["Time (s)", "z_FL (m)", "z_FR (m)", "z_RL (m)", "z_RR (m)"]
